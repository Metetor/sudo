...
int main(int argc, char* argv[]) {
    ...
    if (argc < 2) {
        ...
        std::cout << "-m <difficulty>   Set the difficulty level for generated games (1-3)" << std::endl;
        std::cout << "-r <range>        Set the range of empty cells in generated games (e.g., 20-55)" << std::endl;
        std::cout << "-u                Generate games with unique solutions" << std::endl;
        return 1;
    }
    int option;
    int final_count = 0;
    ...
    if ((flagM || flagR || flagS || flagU) && !flagN)
    {
        std::cerr << "-m,-r,-s,-u must follows -n" << std::endl;
        return 1;
    }
